...
impl Cmd {
    ...
    pub fn read(
        ...
        match graphemes.next() {
            ...
            Some("h") => parse_help_cmd(&mut graphemes, address.as_ref()),
            Some("i") => {
...
fn parse_help_cmd(
    ...
    // Copy rest of cmd line, if any, as initial command for Help.
    let mut remainder = graphemes
        .peeking_take_while(|gr| Eol::from_line(*gr).is_none())
        .collect::<String>();
    let initial_cmd = if remainder.is_empty() {
        None
    } else {
        remainder.push_str(graphemes.next().expect("next graph is newline"));
        Some(remainder)
    };
...
pub(crate) fn parse_substitute_cmd(
    ...
    let (cmd, sfx) = loop {
        ...
        if !more_lines {
            let target = parse_target_match(&mut graphemes)?;
...
fn parse_target_match(
    graphemes: &mut Peekable<Graphemes<'_>>,
) -> Result<TargetMatch, Error> {
    if let Some(n) = parse_usize(graphemes)? {
        let n = n.checked_sub(1).ok_or(Error::InvalidTargetMatch)?;
        return Ok(TargetMatch::Single(n));
    }

    if graphemes.next_if_eq(&"a").is_some() {
        return Ok(TargetMatch::All);
    }

    Ok(TargetMatch::Single(0))
}
...
fn parse_global_command_list(
    ...
    let mut graphemes = cmd_line.graphemes(true);
    loop {
        match graphemes.next() {
            Some("\\") => {
                let escaped_gr =
                    graphemes.next().ok_or(Error::TrailingBackslash)?;
                commands.push_str(escaped_gr);
            }
            Some(gr) => {
            ...
            None => {
                ...
                let bytes_read = input
                    .accept_line(&mut cmd_line, Some(&line_read_options))
                    .map_err(|e| Error::ReadCommand {
                        source: Some(Box::new(e)),
                    })?;
                if bytes_read == 0 {
                    return Err(Error::ReadCommand {
                        source: Some(Box::new(Error::UnexpectedEndOfInput)),
                    });
                }
                graphemes = cmd_line.graphemes(true);
            }
        }
    }
...
fn parse_global_cmd(
    ...
    let commands =
        parse_global_command_list(graphemes.collect::<String>(), input)?;
...
mod tests {
    ...
    fn eval_positive_offset() {
        ...
        let res = compute_line_offset(&mut input).expect("no error");
        assert_eq!(res, 3);
    ...
    #[test]
    fn eval_dot_addr_empty_buffer() {
        let mut buffer = EditBuffer::new();
        let err = eval_address(
            &mut ".d\n".graphemes(true).peekable(),
            &mut buffer,
            &mut None,
        )
        .expect_err("invalid address");
        assert_matches!(err, Error::InvalidAddress);
    }
    ...
    fn eval_simple_comma_addr() {
        ...
        let mut input = "1,2p\n".graphemes(true).peekable();
        let res =
            eval_address(&mut input, &mut buffer, &mut None).expect("no error");
    ...
    fn parse_valid_lone_cmd() {
        let mut cmd_line = "\r\n".graphemes(true).peekable();
        let res = parse_simple_cmd(&mut cmd_line, None, Cmd::Quit)
            .expect("no error")
            .expect("Some cmd");
    ...
    fn parse_no_args_n_print_suffix() {
        ...
        assert!(
            matches!(res, (Cmd::Delete(None), Some(pr_sfx)) if pr_sfx == expected_sfx)
        );
    }
    ...
    fn parse_print_suffix_p() {
        let mut graphs = "p\r\n".graphemes(true).peekable();
        let res = parse_print_suffix(&mut graphs)
            .expect("no error")
            .expect("Some cmd");
        assert_matches!(
            res,
            a if a == PrintSuffix { ..Default::default() });
    }
    ...
    #[test]
    fn parse_append_cmd_no_addr_trailing_space() {
        let mut input = "a \r\n".as_bytes();
        let (cmd, pr_sfx) =
            Cmd::read(&mut input, &mut EditBuffer::new(), &mut None)
                .expect("no error")
    ...
    fn parse_insert_cmd_no_addr() {
        // note trailing space to test parsing
        let mut input = "i \r\n".as_bytes();
        let (cmd, pr_sfx) =
            Cmd::read(&mut input, &mut EditBuffer::new(), &mut None)
                .expect("no error")
    ...
    fn parse_overwrite_cmd_no_addr() {
        // note trailing space to test parsing path
        let mut input = "o \r\n".as_bytes();
        let (cmd, pr_sfx) =
            Cmd::read(&mut input, &mut EditBuffer::new(), &mut None)
                .expect("no error")
    ...
    fn parse_open_no_print_suffix() {
        ...
        let res = parse_edit_cmd(&mut cmd_line, None)
            .expect("no error")
            .expect("Some cmd");
    ...
    fn parse_substitute_missing_delimiter() {
        ...
        .expect_err("missing delimiter");
        assert_matches!(err, Error::MissingPatternDelimiter);
        ...
        let err = parse_substitute_cmd(
            &mut "/, */, foo".graphemes(true).peekable(),
            &mut "".as_bytes(),
            &buffer,
            &mut None,
            None,
    ...
    fn parse_substitute_indexed() {
        ...
        assert_eq!(sub.replacement, ".");
        assert_eq!(sub.target, TargetMatch::Single(2));
    }

    #[test]
    fn parse_substitute_replace_all() {
        let mut cmd_line = "/[^01]*/./a\r\n".graphemes(true).peekable();
        let buffer = EditBuffer::with_lines(["1\n", "2", "3", "4", "5", "6"]);
        let mut prev_pattern = None;
        let res = parse_substitute_cmd(
            &mut cmd_line,
        ...
        assert_eq!(sub.pattern.as_str(), "[^01]*");
        assert_eq!(sub.replacement, ".");
        assert_eq!(sub.target, TargetMatch::All);
    }

    #[test]
    fn parse_substitute_invalid_target_match() {
        let buffer = EditBuffer::with_lines(["1\n", "2", "3"]);
        let err = parse_substitute_cmd(
            &mut "/[^01]*/.\\\n".graphemes(true).peekable(),
            &mut "blah\\\n./0\n".as_bytes(),
        ...
        .expect_err("invalid target match");
        assert_matches!(err, Error::InvalidTargetMatch);
    }

    #[test]
    fn parse_substitute_conflicting_flags() {
        let mut cmd_line = "/[^01]*/./a1\r\n".graphemes(true).peekable();
        let buffer = EditBuffer::with_lines(["1\n", "2", "3", "4", "5", "6"]);
        let mut prev_pattern = None;
        let res = parse_substitute_cmd(
            &mut cmd_line,
    ...
    fn parse_line_number_cmd() {
        ...
        let mut input1 = "=\n".as_bytes();
        let mut input2 = ".=\n".as_bytes();
        let res = Cmd::read(&mut input1, &mut buffer, &mut None)
            .expect("no error")
            .expect("Some cmd");
        assert_matches!(res, (Cmd::LineNumber(None), None));
        let res = Cmd::read(&mut input2, &mut buffer, &mut None)
            .expect("no error")
            .expect("Some cmd");
    ...
    fn parse_write_as_cmd_with_address() {
        ...
        let res = parse_write_as_cmd(&mut cmd_line, Some(0..5))
            .expect("no error")
            .expect("Some cmd");
        assert!(
    ...
    fn show_cmd_no_args_parses_as_show_diff_cmd() {
        let mut cmd_line = "\n".graphemes(true);
        let res = parse_show_cmd(&mut cmd_line, None)
            .expect("no error")
            .expect("show diff cmd");
    ...
    fn parse_valid_page_down_cmd() {
        ...
        let mut buffer =
            EditBuffer::with_lines(["1\r\n", "2", "3", "4", "5", "6"]);
        let res = Cmd::read(&mut input, &mut buffer, &mut None)
            .expect("no error")
            .expect("Some cmd");
        assert!(
    ...
    fn parse_newline_no_print_suffix() {
        let mut cmd_line = "\n".graphemes(true);
        let res = parse_newline_cmd(&mut cmd_line, None)
            .expect("no error")
            .expect("Some cmd");
        assert_matches!(res, (_, None));
    }
    ...
    #[test]
    fn parse_justify_cmd_line_width_no_fill() {
        let expected_addr = 1..5;
        let expected_sfx =
            PrintSuffix { enumerate: true, ..Default::default() };
    ...
    #[test]
    fn parse_help_cmd_address_is_error() {
        let mut input = "\n".graphemes(true).peekable();
        let err = parse_help_cmd(&mut input, Some(1..2).as_ref())
            .expect_err("unexpected address");
        assert_matches!(err, Error::UnexpectedAddress);
    }
